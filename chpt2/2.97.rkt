...
(define (install-polynomial-package)
  ...
  (define (quotient-polys p1 p2) (quotient (div-polynomials p1 p2)))
  ...
  (define (quotient-terms l1 l2) (quotient (div-terms l1 l2)))
  ...
  (define (pseudoremainder-terms l1 l2)
    (let* ((o1 (order (first-term l1)))
           ...
           (integerizing-term (make-term 0 integerizing-factor))
           (modified-p (mul-term-by-all-terms integerizing-term l1)))
      (remainder-terms modified-p l2)))
  ...
  (define (gcd-coefficients terms)
    (if (empty-termlist? (rest-terms terms))
        (coeff (first-term terms))
        (greatest-common-divisor (coeff (first-term terms))
                                 (gcd-coefficients (rest-terms terms)))))

  (define (simplify-coefficients term-list)
    (let ((coeffs-gcd (gcd-coefficients term-list)))
      (mul-term-by-all-terms (make-term 0 (/ coeffs-gcd))
                             term-list)))

  (define (gcd-terms l1 l2)
    (if (empty-termlist? l2)
        (simplify-coefficients l1)
  ...
  (define (reduce-polynomials p1 p2)
    (if (same-variable? (variable p1) (variable p2))
  ...
  (define (reduce-terms n d)
    (let* ((g (gcd-terms n d))
           ...
           (integerizing-factor (expt c (+ 1 o1 (- o2))))
           (integerizing-term (make-term 0 integerizing-factor))
           (integerized-n (mul-term-by-all-terms integerizing-term n))
           (integerized-d (mul-term-by-all-terms integerizing-term d)))
      (list (quotient-terms integerized-n g)
            (quotient-terms integerized-d g))))
  ...
  (put 'quotient '(polynomial polynomial) (lambda (p1 p2) (tag (quotient-polys p1 p2))))
  ...
  (put 'reduce '(polynomial polynomial)
       (lambda (p1 p2)
         (let ((reduced (reduce-polynomials p1 p2)))
           (list (tag (car reduced))
                 (tag (cadr reduced))))))
